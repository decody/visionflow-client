...
import { ArrowLeft, Bold, ChevronDown, ChevronUp, FileText, Globe, Heading1, Heading2, Italic, Link2, List, MapPin, Mail, MoreHorizontal, Paperclip, Send, Timer, Underline, } from 'lucide-react';
import Link from 'next/link';
import { useState } from 'react';
import styles from './partnership-detail-page.module.css';
const TABS = [
    { count: null, key: 'proposal', label: '제안 본문' },
    { count: null, key: 'reply', label: '회신' },
    { count: 6, key: 'history', label: '히스토리' },
    { count: null, key: 'memo', label: '내부 메모' },
];
const TOOLBAR = [
    [
...
export function PartnershipDetailPage({ id }) {
      ...
      <div className={styles.contentGrid}>
        ...
        <article className={styles.composer}>
          ...
          <div className={styles.toolbar}>
            {TOOLBAR.map((group, gi) => (<div className={styles.toolbarGroup} key={`tg-${gi}`}>
                {group.map((tool) => {
          ...
          <textarea className={styles.editor} onChange={(event) => setReply(event.target.value)} rows={12} value={reply}/>
