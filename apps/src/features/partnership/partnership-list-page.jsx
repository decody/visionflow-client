...
import { AlertTriangle, CalendarDays, Check, ChevronDown, Download, Filter, Handshake, MoreHorizontal, Search, Timer, TrendingUp, } from 'lucide-react';
...
const STATUS_TABS = [
    { count: 124, key: 'all', label: '전체' },
    { count: 3, key: 'new', label: 'NEW' },
    { count: 14, key: 'progress', label: '진행' },
    { count: 8, key: 'negotiating', label: '협상' },
    { count: 67, key: 'closed', label: '체결' },
    { count: 32, key: 'rejected', label: '거절' },
];
const STATUS_LABEL = {
    closed: '체결',
    negotiating: '협상',
    ...
    return (<div className={styles.page}>
        ...
        {KPIS.map((kpi) => {
            ...
            return (<article className={styles.kpiCard} key={kpi.label}>
              ...
              <p className={styles.kpiCaption}>
                {kpi.delta ? (<span className={`${styles.kpiDelta} ${kpi.delta.direction === 'up' ? styles.deltaUp : ''}`}>
                    ↑
                  </span>) : null}
                {kpi.caption}
              </p>
      ...
      <div className={styles.toolbar}>
        <label className={styles.searchField}>
          ...
          <input className={styles.searchInput} placeholder="회사명, 담당자, 제휴 유형 검색" type="search"/>
        </label>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {ROWS.map((row) => (<tr key={row.id}>
                  ...
                  <td>
                    <span className={`${styles.dealBadge} ${styles[`deal_${row.dealType}`]}`}>
                      {DEAL_LABEL[row.dealType]}
                    </span>
                  </td>
                  <td>
                    <Link className={styles.companyCell} href={ROUTES.ADMIN.PARTNERSHIP.DETAIL(row.id)}>
...
function SlaBadge({ sla }) {
    if (sla.kind === 'done') {
        return (<span className={`${styles.slaBadge} ${styles.sla_done}`}>
    ...
    return (<span className={`${styles.slaBadge} ${styles.sla_safe}`}>
      <Timer aria-hidden="true" size={11}/>
      {sla.label}
    </span>);
}
