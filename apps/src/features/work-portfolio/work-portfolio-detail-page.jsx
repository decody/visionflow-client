...
import { AlertTriangle, CheckCircle2, ChevronDown, ChevronRight, Circle, ClipboardList, ExternalLink, Eye, FileText, Hash, History, Image as ImageIcon, List as ListIcon, Lock, Monitor, MoreHorizontal, Paperclip, RefreshCw, Send, Smartphone, Tablet, Upload, } from 'lucide-react';
...
function StatusPill({ status }) {
    if (status === 'done') {
        return (<span className={`${styles.sectionStatus} ${styles.statusDone}`}>
        <CheckCircle2 aria-hidden="true" size={11} strokeWidth={2.5}/>완료
      </span>);
    }
    if (status === 'optional') {
        return (<span className={`${styles.sectionStatus} ${styles.statusOptional}`}>
        <Circle aria-hidden="true" size={9} strokeWidth={2.5}/>선택
      </span>);
    }
    return (<span className={`${styles.sectionStatus} ${styles.statusEmpty}`}>
      <AlertTriangle aria-hidden="true" size={11} strokeWidth={2.5}/>미완성
    </span>);
}
export function WorkPortfolioDetailPage({ id }) {
    const [disclosure, setDisclosure] = useState('B');
    ...
    return (<div className={styles.page}>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.formColumn}>
            ...
            <div className={styles.sectionBody}>
              <div className={styles.fieldRow2}>
                <div className={styles.field}>
                  ...
                  <input className={styles.input} defaultValue="brand-15-season-campaign" id="slug" type="text"/>
                </div>
                <div className={styles.field}>
              ...
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <input className={styles.input} defaultValue="Brand 1.5 — 시즌 캠페인 광고 이미지" id="title" type="text"/>
              </div>
              ...
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <textarea className={`${styles.input} ${styles.textarea}`} defaultValue="시즌 캠페인용 30컷 광고 이미지를 LoRA 학습 + 4채널 동시 운영으로 제작" id="short-desc" maxLength={80}/>
              ...
              <div className={styles.fieldRow3}>
                ...
                <div className={styles.field}>
                  ...
                  <select className={`${styles.input} ${styles.select}`} defaultValue="6" id="duration">
          ...
          <section className={styles.sectionCard}>
            ...
            <div className={styles.sectionBody}>
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <input className={styles.input} defaultValue="SL Cosmetics Co." id="client-name" type="text"/>
              </div>
              ...
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <div className={styles.disclosureGrid}>
                  {DISCLOSURE_OPTIONS.map((opt) => (<button className={`${styles.disclosureCard} ${disclosure === opt.key ? styles.disclosureCardActive : ''}`} key={opt.key} onClick={() => setDisclosure(opt.key)} type="button">
              ...
              <div className={styles.fieldRow2}>
                ...
                <div className={styles.field}>
                  ...
                  <input className={styles.input} defaultValue="2026-04-22" id="consent-date" type="date"/>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className={styles.sectionBody}>
              ...
              <div className={styles.fieldRow2}>
                <div className={styles.field}>
                  ...
                  <input className={styles.input} defaultValue="brand15_og_1200x630.webp" id="og-image" type="text"/>
                </div>
                <div className={styles.field}>
                  ...
                  <input className={styles.input} defaultValue="전환율 +47%" id="key-metric" type="text"/>
                </div>
              </div>
            </div>
          </section>
            ...
            <header className={styles.sectionHeader}>
              ...
              <div className={styles.sectionRight}>
                <div className={styles.mdSwitch}>
                  <button className={`${styles.mdSwitchBtn} ${mdMode === 'edit' ? styles.mdSwitchActive : ''}`} onClick={() => setMdMode('edit')} type="button">
                    편집
                  </button>
                  <button className={`${styles.mdSwitchBtn} ${mdMode === 'preview' ? styles.mdSwitchActive : ''}`} onClick={() => setMdMode('preview')} type="button">
                    미리
                  </button>
                </div>
                <StatusPill status="done"/>
          ...
          {COLLAPSED_SECTIONS.map((section) => (<section className={`${styles.sectionCard} ${styles.sectionCardCollapsed}`} key={section.num}>
              <header className={`${styles.sectionHeader} ${styles.sectionHeaderCollapsed}`}>
                <span className={`${styles.sectionThumb} ${section.status === 'optional'
                ...
                <div className={styles.sectionRight}>
                  ...
                  <button aria-label={`${section.title} 펼치기`} className={styles.sectionToggle} type="button">
          ...
          <div className={styles.previewHeader}>
            ...
            <div className={styles.previewActions}>
              <div className={styles.deviceToggle}>
                <button aria-label="데스크톱 미리보기" className={`${styles.deviceBtn} ${device === 'desktop' ? styles.deviceBtnActive : ''}`} onClick={() => setDevice('desktop')} type="button">
                  <Monitor aria-hidden="true" size={13}/>
                </button>
                <button aria-label="모바일 미리보기" className={`${styles.deviceBtn} ${device === 'mobile' ? styles.deviceBtnActive : ''}`} onClick={() => setDevice('mobile')} type="button">
                  <Smartphone aria-hidden="true" size={13}/>
                </button>
                <button aria-label="태블릿 미리보기" className={`${styles.deviceBtn} ${device === 'tablet' ? styles.deviceBtnActive : ''}`} onClick={() => setDevice('tablet')} type="button">
          ...
          <div className={styles.previewIframe}>
            ...
            <div className={styles.previewContent}>
              ...
              <div className={styles.challengeList}>
                {CHALLENGE_CARDS.map((card) => (<div className={styles.challengeCard} key={card.num}>
