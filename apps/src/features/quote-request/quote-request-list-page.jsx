...
import { AlertTriangle, CalendarDays, Check, ChevronDown, Columns3, Download, Layers, MoreHorizontal, Search, Timer, } from 'lucide-react';
...
const STATUS_TABS = [
    { count: 342, key: 'all', label: '전체' },
    { count: 4, key: 'new', label: 'NEW', tone: 'red' },
    { count: 8, key: 'review', label: '검토', tone: 'amber' },
    { count: 14, key: 'sent', label: '견적 전달', tone: 'blue' },
    { count: 6, key: 'negotiating', label: '협상', tone: 'purple' },
    { count: 180, key: 'won', label: '수주', tone: 'green' },
    { count: 130, key: 'rejected', label: '거절', tone: 'gray' },
];
const STATUS_LABEL = {
    negotiating: '협상',
    new: 'NEW',
...
export function QuoteRequestListPage() {
    ...
    return (<div className={styles.page}>
      <section aria-label="요약 지표" className={styles.kpiRow}>
        {KPIS.map((kpi) => (<article className={`${styles.kpiCard} ${kpi.highlight ? styles.kpiCardHighlight : ''}`} key={kpi.label}>
            <header className={styles.kpiTop}>
              <span className={`${styles.kpiLabel} ${styles[`kpiLabel_${kpi.tone}`]}`}>
                {kpi.label}
              </span>
              {kpi.delta === 'up' ? (<span className={styles.kpiArrowUp} aria-hidden="true">
                  ↑
                </span>) : kpi.delta === 'warn' ? (<AlertTriangle aria-hidden="true" className={styles.kpiArrowWarn} size={12}/>) : null}
            </header>
      ...
      <div className={styles.toolbar}>
        <div className={styles.tabsBar} role="tablist" aria-label="상태">
          {STATUS_TABS.map((tab, index) => (<button aria-selected={index === 0} className={`${styles.tab} ${index === 0 ? styles.tabActive : ''}`} key={tab.key} role="tab" type="button">
              {'tone' in tab && tab.tone ? (<span aria-hidden="true" className={`${styles.tabDot} ${styles[`tabDot_${tab.tone}`]}`}/>) : null}
              <span>{tab.label}</span>
        ...
        <label className={styles.searchField}>
          ...
          <input className={styles.searchInput} placeholder="회사명, 담당자, 서비스로 검색" type="search"/>
        </label>
      ...
      <article className={styles.tableCard}>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {ROWS.map((row) => (<tr key={row.id}>
                  ...
                  <td>
                    <span className={`${styles.serviceBadge} ${styles[`svc_${row.service.replace('-', '_')}`]}`}>
                      {SERVICE_LABEL[row.service]}
                    </span>
                  </td>
                  <td>
                    <Link className={styles.companyCell} href={ROUTES.ADMIN.QUOTE_REQUEST.DETAIL(row.id)}>
                  ...
                  <td className={styles.budgetCell}>
                    ...
                    {row.budget.kind === 'confirmed' ? (<span className={styles.budgetConfirmed}>
                        <Check aria-hidden="true" size={10} strokeWidth={3}/>
                        확정
                      </span>) : (<span className={styles.budgetRange}>범위 추정</span>)}
                  </td>
                  <td>
        ...
        <footer className={styles.tableFooter}>
          ...
          <nav aria-label="페이지" className={styles.pagination}>
            ...
            {PAGINATION.map((page, index) => (<button aria-current={page === '1' ? 'page' : undefined} className={`${styles.pageNum} ${page === '1' ? styles.pageNumActive : ''}`} disabled={page === '⋯'} key={`${page}-${index}`} type="button">
                {page}
              </button>))}
...
function SlaBadge({ sla }) {
    if (sla.kind === 'sent') {
        return (<span className={styles.slaSent}>
    ...
    if (sla.kind === 'overdue') {
        return (<span className={`${styles.slaBadge} ${styles.sla_overdue}`}>
        <AlertTriangle aria-hidden="true" size={11}/>
        {sla.label}
      </span>);
    }
    if (sla.kind === 'warn') {
        return (<span className={`${styles.slaBadge} ${styles.sla_warn}`}>
        <Timer aria-hidden="true" size={11}/>
        {sla.label}
      </span>);
    }
    return (<span className={`${styles.slaBadge} ${styles.sla_safe}`}>{sla.label}</span>);
}
