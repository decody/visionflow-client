...
const ndaSteps = [
  { label: '양식 접수 및 1차 검토', status: 'done' as const },
  {
    label: '법무 의견 검토 (제5조 손해배상 한도, 제8조 분쟁 관할)',
    status: 'progress' as const,
  },
  { label: '양사 합의안 협의', status: 'pending' as const },
  { label: '서명 완료 → RFP 공유', status: 'pending' as const },
];
...
function StateLabel({ icon, text }: { icon: string; text: string }) {
  return (
...
function BoardMock() {
  return (
    <div aria-hidden="true" className={styles.boardMock}>
      <div className={styles.boardMockHeader}>번호 제목 작성자 날짜 조회 답변</div>
      {boardRows.map((r) => (
        <div
          className={`${styles.boardMockRow} ${
            r.highlight ? styles.boardMockRowHighlight : ''
          }`}
          key={r.no}
        >
...
function ModalScene({ children }: { children: React.ReactNode }) {
  return (
    <div className={styles.modalScene}>
      <BoardMock />
...
function PasswordModal({ variant }: { variant: 'default' | 'error' }) {
  ...
  return (
    <div className={styles.modal} role="dialog">
      ...
      <div className={styles.modalBody}>
        <div
          aria-hidden="true"
          className={`${styles.modalIcon} ${isError ? styles.modalIconError : ''}`}
        >
          🔒
        </div>
        <div className={styles.modalText}>
          ...
          <p className={styles.modalSub}>
            {isError ? (
              '다시 시도해 주세요. 비밀번호는 작성 시 본인이 설정한 값입니다.'
            ) : (
              <>
        ...
        <div className={styles.modalField}>
          ...
          <div
            className={`${styles.modalInputWrap} ${
              isError ? styles.modalInputWrapError : styles.modalInputWrapFocus
            }`}
          >
            {isError ? (
              <input
              ...
              <span aria-hidden="true" className={styles.modalDots}>
                {Array.from({ length: 6 }).map((_, i) => (
                  <span className={styles.modalDot} key={i} />
                ))}
            ...
            <button
              ...
              className={`${styles.modalEyeBtn} ${
                isError ? styles.modalEyeBtnError : ''
              }`}
              type="button"
            >
              {isError ? '⚠️' : '👁'}
...
function LockoutModal() {
  return (
    <div className={`${styles.modal} ${styles.modalLockout}`} role="dialog">
        ...
        <article className={styles.answerCard}>
          ...
          <div className={styles.answerBody}>
            ...
            <div className={styles.ndaBox}>
              ...
              {ndaSteps.map((s) => (
                <div className={styles.ndaRow} key={s.label}>
                  <span
                    aria-hidden="true"
                    className={`${styles.ndaMarker} ${
                      s.status === 'done'
                  ...
                  <span
                    className={`${styles.ndaStatus} ${styles[`ndaStatus_${s.status}`] ?? ''}`}
                  >
      ...
      <section aria-label="비밀 댓글" className={styles.commentsCard}>
        ...
        <form className={styles.commentForm}>
          ...
          <textarea
            ...
            maxLength={500}
            placeholder="답변 내용을 입력하세요. 작성자와 운영팀만 볼 수 있습니다."
            required
          />
          <div className={styles.commentFormFooter}>
            ...
            <button
              className={`${styles.commentSubmit} ${styles.commentSubmitSecret}`}
              type="submit"
            >
    ...
    <div className={styles.page}>
      <Container>
        <section className={styles.stateSection}>
          <StateLabel
            icon="🔒"
            text="STATE A · 비밀글 클릭 직후 — 비밀번호 입력 모달"
          />
          <ModalScene>
        ...
        <section className={styles.stateSection}>
          <StateLabel
            icon="⚠️"
            text="STATE A2 · 비밀번호 오류 — input error state + 시도 횟수 안내"
          />
          <ModalScene>
        ...
        <section className={styles.stateSection}>
          <StateLabel
            icon="✓"
            text="STATE B · 인증 후 비밀글 본문 — 비공개 banner + NDA 진행중 + 비밀 댓글 default"
          />
          <SecretPostView />
        </section>
        ...
        <section className={styles.stateSection}>
          <StateLabel
            icon="🚫"
            text="STATE C · 5회 연속 실패 — 5분 lockout + 운영팀 직접 연락 권유"
          />
          <ModalScene>
            <LockoutModal />
          </ModalScene>
