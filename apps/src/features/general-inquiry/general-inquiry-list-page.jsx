...
import { CalendarDays, Check, ChevronDown, Download, MoreHorizontal, Search, Tag, } from 'lucide-react';
import Link from 'next/link';
import { useTopbar } from '../../components/layout/topbar-context';
import styles from './general-inquiry-list-page.module.css';
const STATUS_TABS = [
    { count: 186, key: 'all', label: '전체' },
    { count: 5, key: 'unanswered', label: '미답변' },
    { count: 14, key: 'reviewed', label: '확인' },
    { count: 167, key: 'done', label: '답변 완료' },
];
const STATUS_LABEL = {
    done: '완료',
    reviewed: '확인',
    ...
    return (<div className={styles.page}>
      ...
      <div className={styles.toolbar}>
        <label className={styles.searchField}>
          ...
          <input className={styles.searchInput} placeholder="제목, 작성자, 이메일로 검색" type="search"/>
        </label>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {ROWS.map((row) => (<tr key={row.id}>
                  ...
                  <td>
                    <span className={`${styles.catBadge} ${styles[`cat_${row.category}`]}`}>
                      {CATEGORY_LABEL[row.category]}
                    </span>
                  </td>
                  <td className={styles.titleCell}>
                    <Link className={styles.titleLink} href={ROUTES.ADMIN.GENERAL_INQUIRY.DETAIL(row.id)}>
                      {row.title}
                    </Link>
