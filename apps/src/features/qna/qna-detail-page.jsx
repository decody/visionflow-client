...
import { ArrowLeft, ArrowRight, Bold, ChevronDown, ChevronUp, Code2, Eye, FileText, Heading1, Heading2, Heading3, Image as ImageIcon, Italic, Link2, List, ListOrdered, MoreHorizontal, Paperclip, Quote, Strikethrough, Timer, Underline, } from 'lucide-react';
...
const TABS = [
    { count: null, key: 'body', label: '본문' },
    { count: null, key: 'compose', label: '답변 작성' },
    { count: 12, key: 'log', label: '작업 로그' },
];
...
export function QnaDetailPage({ id }) {
    ...
    return (<div className={styles.page}>
      ...
      <article className={styles.questionCard}>
        ...
        <footer className={styles.attachmentsRow}>
          ...
          {ATTACHMENTS.map((file) => (<a className={styles.attachmentChip} href="#" key={file.label}>
      ...
      <nav aria-label="섹션" className={styles.tabsBar} role="tablist">
        {TABS.map((tab) => (<button aria-selected={activeTab === tab.key} className={`${styles.tab} ${activeTab === tab.key ? styles.tabActive : ''}`} key={tab.key} onClick={() => setActiveTab(tab.key)} role="tab" type="button">
            {tab.label}
      ...
      <article className={styles.composer}>
        ...
        <div className={styles.toolbar}>
          {TOOLBAR_GROUPS.map((group, gi) => (<div className={styles.toolbarGroup} key={`group-${gi}`}>
              {group.map((tool) => {
                const Icon = tool.icon;
                return (<button aria-label={tool.label} className={styles.toolButton} key={tool.label} type="button">
        ...
        <textarea className={styles.editor} onChange={(event) => setReply(event.target.value)} rows={14} value={reply}/>
