...
import { AlertTriangle, CalendarDays, Check, ChevronDown, Download, Lock, MoreHorizontal, Plus, Search, SlidersHorizontal, Timer, } from 'lucide-react';
    ...
    return (<div className={styles.page}>
      ...
      <div className={styles.tabsBar} role="tablist" aria-label="답변 상태">
        {STATUS_TABS.map((tab, index) => {
            ...
            return (<button aria-selected={index === 0} className={`${styles.tab} ${index === 0 ? styles.tabActive : ''}`} key={tab.key} role="tab" type="button">
      ...
      <div className={styles.toolbar}>
        <label className={styles.searchField}>
          ...
          <input className={styles.searchInput} placeholder="제목, 작성자, 카테고리로 검색" type="search"/>
        </label>
      ...
      <article className={styles.tableCard}>
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            ...
            <tbody>
              {INQUIRIES.map((row) => (<tr key={row.id}>
                  <td className={styles.checkboxCell}>
                    <input aria-label={`${row.id} 선택`} type="checkbox"/>
                  </td>
                  <td>
                    <span className={`${styles.statusBadge} ${row.status === 'done' ? styles.status_done : styles.status_pending}`}>
                      {row.status === 'done' ? '완료' : '대기'}
                    </span>
                  </td>
                  <td>
                    <span className={`${styles.categoryBadge} ${styles[`cat_${row.category.replace('-', '_')}`]}`}>
                      {CATEGORY_LABEL[row.category]}
                    </span>
                  </td>
                  <td className={styles.titleCell}>
                    <Link className={styles.titleLink} href={ROUTES.ADMIN.QNA.DETAIL(row.id)}>
                      {row.private ? (<Lock aria-hidden="true" className={styles.titleLock} size={13}/>) : null}
        ...
        <footer className={styles.tableFooter}>
          ...
          <nav aria-label="페이지" className={styles.pagination}>
            ...
            {PAGINATION_PAGES.map((page, index) => (<button aria-current={page === '1' ? 'page' : undefined} className={`${styles.pageNum} ${page === '1' ? styles.pageNumActive : ''}`} disabled={page === '⋯'} key={`${page}-${index}`} type="button">
                {page}
              </button>))}
