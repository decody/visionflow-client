        ...
        <Container>
          <div className={styles.tabsCenter}>
            <div className={styles.tabsBar} role="tablist">
              {tabs.map((t) => (<Link className={`${styles.tab} ${t.active ? styles.tabActive : ''}`} href={t.href} key={t.label}>
                  {t.label}
                  {'count' in t && t.count !== undefined ? (<span className={styles.tabCount}>{t.count}</span>) : null}
                </Link>))}
            </div>
          </div>
        </Container>
        ...
        <Container>
          <div className={styles.boardToolbar}>
            <div className={styles.categoryGroup}>
              {categories.map((c) => (<button className={`${styles.categoryChip} ${c.active ? styles.categoryChipActive : ''}`} key={c.label} type="button">
                  {c.label}
                  {'count' in c && c.count !== undefined ? (<span className={styles.categoryCount}>{c.count}</span>) : null}
                </button>))}
            </div>
            ...
            <div className={styles.searchBox}>
              ...
              <input aria-label="검색어" className={styles.searchInput} placeholder="검색어를 입력하세요" type="search"/>
            </div>
          ...
          <div className={styles.table} role="table">
            ...
            {posts.map((p, i) => (<div className={`${styles.row} ${p.isNotice ? styles.rowNotice : ''}`} key={`${p.no}-${i}`} role="row">
                <span className={styles.cellNo}>
                  {p.isNotice ? <span className={styles.noticeBadge}>공지</span> : p.no}
                </span>
                <span className={styles.cellTitle}>
                  {p.status ? (<span className={`${styles.statusBadge} ${styles[statusClass[p.status]]}`}>
                      {statusLabel[p.status]}
                    </span>) : null}
                  <span className={`${styles.titleText} ${p.isNotice ? styles.titleTextNotice : ''}`}>
                    {p.locked ? '🔒 ' : ''}
                    {p.title}
                  </span>
          ...
          <nav aria-label="pagination" className={styles.pagination}>
            <button aria-label="이전 페이지" className={`${styles.pageBtn} ${styles.pageBtnNav}`} type="button">
              ‹
            </button>
            {pageNumbers.map((n) => (<button className={`${styles.pageBtn} ${n === 1 ? styles.pageBtnActive : ''}`} key={n} type="button">
                {n}
              </button>))}
            <button aria-label="다음 페이지" className={`${styles.pageBtn} ${styles.pageBtnNav}`} type="button">
              ›
            </button>
          </nav>
        </Container>
          ...
          <form className={styles.formCard}>
            <div className={styles.formRow}>
              <div className={styles.formField}>
                ...
                <input className={styles.formInput} id="qf-name" name="name" placeholder="문의자 이름" required type="text"/>
              </div>
              <div className={styles.formField}>
                <label className={styles.formLabel} htmlFor="qf-email">
                  이메일 <span className={styles.formRequired}>*</span>
                </label>
                <input className={styles.formInput} id="qf-email" name="email" placeholder="you@example.com" required type="email"/>
              </div>
            </div>
            <div className={styles.formField}>
              ...
              <input className={styles.formInput} id="qf-title" name="title" placeholder="문의 제목 (선택)" type="text"/>
            </div>
            <div className={styles.formField}>
              <label className={styles.formLabel} htmlFor="qf-message">
                내용 <span className={styles.formRequired}>*</span>
              </label>
              <textarea className={styles.formTextarea} id="qf-message" name="message" placeholder={'궁금한 점을 자유롭게 작성해 주세요.\n견적·기술·진행 절차·NDA·기타 무엇이든.'} required rows={6}/>
            </div>
        ...
        <Container>
          <div className={styles.hintBanner}>
            ...
            <div className={styles.hintActions}>
              <Link className={`${styles.hintCta} ${styles.hintCtaPrimary}`} href={`${ROUTES.CONTACT}#quote`}>
                견적 문의 →
              </Link>
              <Link className={`${styles.hintCta} ${styles.hintCtaSecondary}`} href={`${ROUTES.CONTACT}#partnership`}>
                제휴 문의 →
              </Link>
            </div>
          </div>
