...
const adminAnswer = {
    ...
    intro: '말씀해 주신 100컷 규모 (제품 60 + 키비주얼 20 + 라이프스타일 20) 작업의 일반적인 일정은 다음과 같습니다.',
    timeline: [
...
const comments = [
    {
        ...
        body: '상세한 답변 정말 감사합니다! 일정과 단가표 모두 명확하게 이해되었습니다. 견적 폼으로 정식 요청 진행하겠습니다.',
        likes: 0,
        hasReply: true,
    },
    {
        ...
        body: '비슷한 규모 작업 의뢰했었는데 실제로 2주 정도 걸렸습니다. 가이드라인 잘 정리되어 있으면 더 빨라질 수 있어요.',
        likes: 5,
        hasReply: true,
    },
    {
        ...
        body: '@이재원 님, 경험 공유 감사합니다! 말씀처럼 가이드라인이 잘 정리되어 있을수록 1단계가 단축되어 전체 일정이 줄어듭니다.',
        likes: 2,
        hasReply: false,
        isStaff: true,
        isReply: true,
      ...
      <Container>
        <nav aria-label="breadcrumb" className={styles.breadcrumbBar}>
          <ol className={styles.breadcrumbList}>
            {breadcrumbs.map((b, i) => (<li className={styles.breadcrumbItem} key={b.label}>
                {b.current ? (<span aria-current="page" className={styles.breadcrumbCurrent}>
                    {b.label}
                  </span>) : (<Link className={styles.breadcrumbLink} href={b.href}>
                    {b.label}
                  </Link>)}
                {i < breadcrumbs.length - 1 ? (<span aria-hidden="true" className={styles.breadcrumbSep}>
                    ›
                  </span>) : null}
              </li>))}
          </ol>
        ...
        <article className={styles.postCard}>
          ...
          <div className={styles.postBody}>
            {post.paragraphs.map((p, i) => (<p className={styles.bodyParagraph} key={i}>
                {p}
              </p>))}
            <p className={styles.attachLabel}>첨부 자료</p>
            {post.attachments.map((a) => (<div className={styles.attachItem} key={a.name}>
          ...
          <article className={styles.answerCard}>
            ...
            <div className={styles.answerBody}>
              ...
              <div className={styles.timelineBox}>
                {adminAnswer.timeline.map((row) => (<div className={styles.timelineRow} key={row.step}>
              ...
              {adminAnswer.paragraphs.map((p, i) => (<p className={styles.bodyParagraph} key={i}>
                  {p}
                </p>))}
              ...
              <div className={styles.feedbackRow}>
                ...
                <div className={styles.feedbackActions}>
                  <button className={`${styles.feedbackBtn} ${styles.feedbackBtnPositive}`} type="button">
                    <span aria-hidden="true">👍</span> 도움됨{' '}
                    <span className={styles.feedbackCount}>{adminAnswer.helpful}</span>
                  </button>
                  <button className={`${styles.feedbackBtn} ${styles.feedbackBtnNeutral}`} type="button">
        ...
        <section aria-label="댓글" className={styles.commentsCard}>
          ...
          <ul className={styles.commentList}>
            {comments.map((c, i) => (<li className={`${styles.commentItem} ${c.isReply ? styles.commentReply : ''}`} key={i}>
                <span aria-hidden="true" className={`${styles.avatar} ${c.isStaff ? styles.avatarPrimary : styles.avatarMuted}`}>
                  {c.initial}
                </span>
                <div className={styles.commentBody}>
                  <div className={styles.commentMetaRow}>
                    <span className={styles.commentAuthor}>{c.author}</span>
                    {c.badge ? (<span className={`${styles.commentBadge} ${c.badge === '운영팀' ? styles.commentBadgeStaff : styles.commentBadgeAuthor}`}>
                        {c.badge}
                      </span>) : null}
                  ...
                  <div className={styles.commentActions}>
                    ...
                    {c.hasReply ? (<button className={styles.commentReplyBtn} type="button">
                        답글 달기
                      </button>) : null}
                  </div>
                </div>
          ...
          <form className={styles.commentForm}>
            ...
            <div className={styles.commentFormRow}>
              <input aria-label="이름" className={styles.commentInput} name="name" placeholder="이름 *" required type="text"/>
              <input aria-label="비밀번호" className={styles.commentInput} name="password" placeholder="비밀번호 (수정·삭제 시 필요)" required type="password"/>
            </div>
            <textarea aria-label="댓글 내용" className={styles.commentTextarea} maxLength={500} name="comment" placeholder="댓글을 입력하세요. (예의있는 표현 부탁드립니다)" required/>
            <div className={styles.commentFormFooter}>
              <label className={styles.privateCheckbox} htmlFor="comment-private">
                <input className={styles.consentInput} id="comment-private" name="private" type="checkbox"/>
