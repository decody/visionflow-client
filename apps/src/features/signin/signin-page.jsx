...
import { ArrowLeft, ArrowRight, Check, Clock, Eye, Info, KeyRound, Lock, Mail, ShieldCheck, User } from 'lucide-react';
    ...
    return (<div className={styles.shell}>
      <section className={styles.brand} aria-hidden="true">
        ...
        <ol className={styles.stepList}>
          {ONBOARDING_STEPS.map((step) => (<li className={`${styles.step} ${styles[`step_${step.status}`]}`} key={step.step}>
              <span className={styles.stepBadge}>
                {step.status === 'done' ? (<Check aria-hidden="true" size={16} strokeWidth={3}/>) : (step.step)}
              </span>
              ...
              {step.status === 'active' ? (<span className={styles.statusActive}>
      ...
      <main className={styles.formArea}>
        <nav className={styles.topNav} aria-label="페이지 보조">
          <div className={styles.langSwitch}>
            <button aria-pressed={language === 'KO'} className={language === 'KO' ? styles.langActive : styles.langInactive} onClick={() => setLanguage('KO')} type="button">
              KO
            </button>
            <span aria-hidden="true">/</span>
            <button aria-pressed={language === 'EN'} className={language === 'EN' ? styles.langActive : styles.langInactive} onClick={() => setLanguage('EN')} type="button">
              EN
            </button>
          </div>
        ...
        <form className={styles.form} onSubmit={(event) => {
          ...
          <div className={styles.field}>
            ...
            <div className={`${styles.inputWrap} ${styles.inputLocked}`}>
              ...
              <input aria-readonly className={styles.input} defaultValue="park.seojun@visionflow.kr" id="signin-email" name="email" readOnly type="email"/>
          ...
          <div className={styles.field}>
            ...
            <div className={styles.inputWrap}>
              ...
              <input autoComplete="name" className={styles.input} defaultValue="박서준" id="signin-name" name="displayName" placeholder="표시할 이름" type="text"/>
            </div>
          ...
          <div className={styles.field}>
            ...
            <div className={styles.inputWrap}>
              ...
              <input autoComplete="new-password" className={styles.input} id="signin-password" name="password" placeholder="••••••••••••••" type={showPassword ? 'text' : 'password'}/>
              <button aria-label={showPassword ? '비밀번호 숨기기' : '비밀번호 표시'} aria-pressed={showPassword} className={styles.iconAction} onClick={() => setShowPassword((value) => !value)} type="button">
            ...
            <div className={styles.policy}>
              ...
              <ul className={styles.policyList}>
                {PASSWORD_RULES.map((rule) => (<li className={`${styles.policyItem} ${rule.hint ? styles.policyItemHint : ''}`} key={rule.text}>
            ...
            <div className={styles.totpBody}>
              ...
              <div className={styles.totpInputs}>
                ...
                <div aria-label="6자리 인증 코드" className={styles.totpDigits} role="group">
                  {TOTP_DIGITS.map((digit, index) => digit === '|' ? (<span aria-hidden="true" className={styles.totpCaret} key="caret">
                        |
                      </span>) : (<span className={`${styles.totpDigit} ${digit ? styles.totpDigitFilled : ''}`} key={`d-${index}`}>
                        {digit}
                      </span>))}
                </div>
              </div>
          ...
          <section className={styles.consent}>
            ...
            <ul className={styles.consentList}>
              {CONSENT_ITEMS.map((item) => (<li className={styles.consentItem} key={item.label}>
                  <label className={styles.checkbox}>
                    <input className={styles.checkboxInput} defaultChecked={item.defaultChecked} type="checkbox"/>
                    <span aria-hidden="true" className={styles.checkboxBox}/>
                    <span className={styles.consentLabel}>
                      {item.label}
                      <span className={item.required ? styles.consentRequired : styles.consentOptional}>
                        {item.required ? '필수' : '선택'}
                      </span>
                    </span>
                  </label>
                  {item.action ? (<a className={styles.smallLink} href="#">
                      {item.action}
                    </a>) : null}
                </li>))}
            </ul>
...
function QrPattern() {
    return (<svg aria-hidden="true" className={styles.qrPattern} height={108} viewBox="0 0 108 108" width={108} xmlns="http://www.w3.org/2000/svg">
      ...
      {[
            ...
            { x: 76, y: 8 },
            { x: 8, y: 76 },
        ].map((corner) => (<g key={`${corner.x}-${corner.y}`}>
      ...
      {[
            ...
            [88, 88],
            [96, 92],
        ].map(([x, y]) => (<rect fill="#0f172a" height={4} key={`${x}-${y}`} width={4} x={x} y={y}/>))}
    </svg>);
}
