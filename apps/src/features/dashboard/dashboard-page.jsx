...
import { ArrowDownRight, ArrowRight, ArrowUpRight, BarChart3, Briefcase, Folder, HelpCircle, MoreHorizontal, PenLine, Reply, TrendingUp, UserPlus, } from 'lucide-react';
    ...
    return (<div className={styles.page}>
      <header className={styles.pageHeader}>
        ...
        <div className={styles.dateFilter} role="tablist" aria-label="기간 선택">
          {DATE_RANGE_TABS.map((tab) => (<button aria-selected={dateRange === tab.key} className={`${styles.dateTab} ${dateRange === tab.key ? styles.dateTabActive : ''}`} key={tab.key} onClick={() => setDateRange(tab.key)} role="tab" type="button">
              {tab.label}
            </button>))}
        </div>
      </header>
      ...
      <section aria-label="주요 지표" className={styles.kpiRow}>
        {KPIS.map((kpi) => {
            const Icon = kpi.icon;
            return (<article className={styles.kpiCard} key={kpi.label}>
              <header className={styles.kpiTop}>
                <span className={styles.kpiLabel}>{kpi.label}</span>
                <span aria-hidden="true" className={`${styles.kpiIcon} ${styles[`kpiIcon_${kpi.tone}`]}`}>
                  <Icon size={18} strokeWidth={2}/>
                </span>
              </header>
              <strong className={styles.kpiValue}>{kpi.value}</strong>
              <div className={styles.kpiDelta}>
                <span className={`${styles.deltaBadge} ${kpi.delta.direction === 'up' ? styles.deltaUp : styles.deltaDown}`}>
                  {kpi.delta.direction === 'up' ? (<ArrowUpRight aria-hidden="true" size={12} strokeWidth={2.5}/>) : (<ArrowDownRight aria-hidden="true" size={12} strokeWidth={2.5}/>)}
                  {kpi.delta.value}
                </span>
      ...
      <section className={styles.chartRow}>
        ...
        ].map((item) => (<li className={styles.legendItem} key={item.label}>
                  <span aria-hidden="true" className={styles.legendDot} style={{ background: item.color }}/>
                  {item.label}
                </li>))}
            </ul>
          </header>
          <div className={styles.chartArea}>
            <ResponsiveContainer height={260} width="100%">
              <LineChart data={LINE_DATA} margin={{ bottom: 8, left: -10, right: 12, top: 8 }}>
                ...
                <XAxis axisLine={false} dataKey="date" fontSize={12} stroke="#94a3b8" tickLine={false}/>
                <YAxis axisLine={false} domain={[0, 200]} fontSize={12} stroke="#94a3b8" tickLine={false} ticks={[0, 50, 100, 150, 200]}/>
            ...
            fontSize: 12,
                ...
                <Line activeDot={{ r: 5 }} dataKey="qna" dot={{ r: 4, strokeWidth: 0 }} name="Q&A" stroke="#004fff" strokeWidth={2.5} type="monotone"/>
                <Line activeDot={{ r: 5 }} dataKey="partnership" dot={{ r: 4, strokeWidth: 0 }} name="Partnership" stroke="#a855f7" strokeWidth={2.5} type="monotone"/>
                <Line activeDot={{ r: 5 }} dataKey="quote" dot={{ r: 4, strokeWidth: 0 }} name="Quote" stroke="#03c75a" strokeWidth={2.5} type="monotone"/>
              </LineChart>
            </ResponsiveContainer>
          </div>
        </article>
          ...
          <div className={styles.donutBody}>
            <div className={styles.donutWrap}>
              <ResponsiveContainer height={200} width={200}>
                <PieChart>
                  <Pie data={[...DONUT_DATA]} dataKey="count" endAngle={-270} innerRadius={64} outerRadius={92} paddingAngle={2} startAngle={90} stroke="none">
                    {DONUT_DATA.map((entry) => (<Cell fill={entry.fill} key={entry.name}/>))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
            ...
            <ul className={styles.donutLegend}>
              {DONUT_DATA.map((entry) => (<li className={styles.donutLegendRow} key={entry.name}>
                  <span aria-hidden="true" className={styles.donutDot} style={{ background: entry.fill }}/>
        ...
        <article className={styles.tableCard}>
          <header className={styles.cardHeader}>
            ...
            <div className={styles.tableTabs} role="tablist">
              {INQUIRY_TABS.map((tab) => (<button aria-selected={inquiryFilter === tab.key} className={`${styles.tableTab} ${inquiryFilter === tab.key ? styles.tableTabActive : ''}`} key={tab.key} onClick={() => setInquiryFilter(tab.key)} role="tab" type="button">
                  {tab.label}
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                {filteredInquiries.map((row) => (<tr key={row.id}>
                    <td>
                      <span className={`${styles.categoryBadge} ${styles[`category_${row.category}`]}`}>
        ...
        <div className={styles.rightCol}>
          <article className={styles.usersCard}>
            ...
            <ul className={styles.userList}>
              {RECENT_USERS.map((user) => (<li className={styles.userRow} key={user.email}>
          ...
          <article className={styles.quickCard}>
            ...
            return (<li key={action.label}>
                    <button className={styles.quickItem} type="button">
                      <span aria-hidden="true" className={`${styles.quickIcon} ${styles[`quickIcon_${action.tone}`]}`}>
