...
import { AlertTriangle, ArrowLeft, ArrowRight, Bell, Check, ChevronRight, Mail, Send, ShieldCheck, Sparkles, Upload, X, } from 'lucide-react';
...
const TOKEN_TIMELINE = [
    { active: true, label: '발송', sub: '지금', tone: 'primary' },
    { label: '리마인더 메일', sub: '12h 후 미수락', tone: 'amber' },
    { label: '경고 알림', sub: '20h 후', tone: 'amber' },
    { label: '토큰 자동 폐기', sub: '24h 후 만료', tone: 'danger' },
];
export function UsersInvitePage() {
    ...
    const [welcomeMessage, setWelcomeMessage] = useState('주현님, VisionFlow 운영 팀에 오신 것을 환영합니다. 시즌 캠페인 운영 + Q&A 답변을 맡아주실 예정입니다. 첫 출근 전까지 가입 완료 부탁드려요. — 이대표');
    useTopbar(() => ({
        action: (<button className={styles.topbarPrimary} type="button">
    ...
    return (<div className={styles.page}>
      ...
      <div className={styles.layout}>
        <div className={styles.formColumn}>
          <Step number="01" title="초대할 이메일" hint="한 번에 여러 명 초대하려면 쉼표로 구분">
            <div className={styles.emailField}>
              ...
              <div className={styles.emailChips}>
                {emails.map((email) => (<span className={styles.emailChip} key={email}>
                    {email}
                    {email.endsWith('@visionflow.kr') ? (<span className={styles.ssoTag}>SSO</span>) : null}
                    <button aria-label={`${email} 제거`} className={styles.chipRemove} onClick={() => handleRemoveEmail(email)} type="button">
                      <X size={11}/>
                    </button>
                  </span>))}
                <input className={styles.emailInput} onBlur={handleAddEmail} onChange={(e) => setEmailInput(e.target.value)} onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ',') {
                e.preventDefault();
                handleAddEmail();
            }
        }} placeholder="이메일 추가…" type="email" value={emailInput}/>
              </div>
            </div>
          ...
          <Step number="02" title="역할 선택" hint="4단계 RBAC · §2.2 권한 매트릭스 기준">
            <div className={styles.roleGrid}>
              {ROLE_OPTIONS.map((option) => (<button aria-pressed={role === option.key} className={`${styles.roleCard} ${role === option.key ? styles.roleCardActive : ''}`} key={option.key} onClick={() => setRole(option.key)} type="button">
                  <header className={styles.roleHeader}>
                    ...
                    {option.notice ? (<span className={styles.roleNotice}>{option.notice}</span>) : null}
                  </header>
                  ...
                  {option.notice2 ? (<span className={styles.roleWarn}>{option.notice2}</span>) : null}
                  {role === option.key ? (<span className={styles.roleCheck}>
          ...
          <Step number="04" title="환영 메시지 (선택)" hint="초대 메일에 함께 발송 · Markdown 일부 지원">
            <textarea className={styles.welcomeTextarea} maxLength={500} onChange={(e) => setWelcomeMessage(e.target.value)} rows={5} value={welcomeMessage}/>
        ...
        <aside className={styles.previewColumn}>
          ...
          <article className={styles.emailPreview}>
            ...
            <div className={styles.emailBody}>
              ...
              {welcomeMessage ? (<div className={styles.emailNote}>
          ...
          <article className={styles.timelineCard}>
            ...
            <ol className={styles.timelineList}>
              {TOKEN_TIMELINE.map((step, index) => (<li className={`${styles.timelineItem} ${step.active ? styles.timelineItemActive : ''}`} key={step.label}>
                  <span aria-hidden="true" className={`${styles.timelineMarker} ${styles[`timelineMarker_${step.tone}`]}`}>
...
function Step({ children, hint, number, title, }) {
