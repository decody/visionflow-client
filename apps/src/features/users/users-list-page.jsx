...
import { Activity, AlertTriangle, Briefcase, ChevronDown, Download, Eye, Globe, MoreHorizontal, Plus, RefreshCcw, Search, Shield, ShieldCheck, Sparkles, User as UserIcon, Users as UsersIcon, } from 'lucide-react';
...
export function UsersListPage() {
    useTopbar(() => ({
        action: (<Link className={styles.topbarPrimary} href={ROUTES.ADMIN.USERS.INVITE}>
    ...
    return (<div className={styles.page}>
        ...
        {KPIS.map((kpi) => {
            ...
            return (<article className={styles.kpiCard} key={kpi.label}>
              <header className={styles.kpiTop}>
                ...
                {'badge' in kpi && kpi.badge ? (<span className={styles.kpiBadge}>
                    <span aria-hidden="true" className={styles.kpiBadgeDot}/>
                    {kpi.badge}
                  </span>) : (<span aria-hidden="true" className={`${styles.kpiIcon} ${styles[`kpiIcon_${kpi.tone}`]}`}>
      ...
      <div className={styles.tabsBar} role="tablist" aria-label="역할 필터">
        {ROLE_FILTERS.map((tab, index) => (<button aria-selected={index === 0} className={`${styles.tab} ${index === 0 ? styles.tabActive : ''}`} key={tab.key} role="tab" type="button">
            {tab.emoji ? (<span aria-hidden="true" className={styles.tabEmoji}>
                {tab.emoji}
              </span>) : null}
            <span className={tab.key !== 'all' && index !== 0
      ...
      <div className={styles.toolbar}>
        <label className={styles.searchField}>
          ...
          <input className={styles.searchInput} placeholder="이름, 이메일로 검색" type="search"/>
        </label>
...
function UserRow({ user }) {
    ...
    return (<tr className={isPending ? styles.rowPending : isInactive ? styles.rowInactive : ''}>
        ...
        <div className={styles.userCell}>
          ...
          <div className={styles.userInfo}>
            <div className={styles.userNameRow}>
              {isPending ? (<span className={styles.userName}>{user.name}</span>) : (<Link className={styles.userName} href={ROUTES.ADMIN.USERS.DETAIL(user.id)}>
                  {user.name}
                </Link>)}
      ...
      <td>
        {isPending ? (<span className={styles.tokenExpiry}>{user.tokenExpiry}</span>) : (<div className={styles.lastLoginCell}>
            <span>{user.lastLoginAbsolute}</span>
            {user.lastLoginLocation ? (<span className={styles.lastLoginSub}>{user.lastLoginLocation}</span>) : null}
          </div>)}
      </td>
      <td>
      ...
      <td className={styles.actionCell}>
        {isPending ? (<button className={styles.resendBtn} type="button">
            <RefreshCcw aria-hidden="true" size={12}/>
            재전송
          </button>) : (<button aria-label="더보기" className={styles.moreButton} type="button">
...
function RoleBadge({ role }) {
    return (<span className={`${styles.roleBadge} ${styles[`role_${role}`]}`}>
...
function TwoFactorBadge({ value }) {
    if (value === 'totp') {
        return (<span className={`${styles.twoFactorBadge} ${styles.totp}`}>
        <Shield aria-hidden="true" size={10}/>
        TOTP
      </span>);
    }
    if (value === 'off') {
        return (<span className={`${styles.twoFactorBadge} ${styles.off}`}>
...
function StatusBadge({ status }) {
    ...
    if (status === 'pending') {
        return (<span className={`${styles.statusBadge} ${styles.statusPending}`}>
        <Eye aria-hidden="true" size={10}/>
        초대 발송
      </span>);
    }
    return (<span className={`${styles.statusBadge} ${styles.statusInactive}`}>
      <span aria-hidden="true" className={styles.statusDot}/>
      비활성
    </span>);
}
