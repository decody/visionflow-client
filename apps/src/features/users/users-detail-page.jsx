...
import { ArrowLeft, Briefcase, Calendar, Check, CircleDot, FileEdit, Globe, IdCard, Image as ImageIcon, Lock, LogOut, MessageCircle, Power, RefreshCcw, Save, ShieldCheck, Trash2, User as UserIcon, Users as UsersIcon, X, } from 'lucide-react';
...
export function UsersDetailPage({ id }) {
    ...
    return (<div className={styles.page}>
      ...
      <section className={styles.profileCard}>
        ...
        <div className={styles.statsBlock}>
          ...
          <div className={styles.statsGrid}>
            {STATS.map((stat) => (<div className={styles.statItem} key={stat.label}>
      ...
      <nav className={styles.tabsBar} aria-label="사용자 상세 섹션">
        {TABS.map((tab) => (<button aria-current={tab.active ? 'page' : undefined} className={`${styles.tab} ${tab.active ? styles.tabActive : ''} ${tab.danger ? styles.tabDanger : ''}`} key={tab.key} type="button">
            <span>{tab.label}</span>
      ...
      <article className={styles.card}>
        ...
        <div className={styles.roleGrid}>
          {ROLE_OPTIONS.map((role) => (<button aria-pressed={selectedRole === role.key} className={`${styles.roleCard} ${selectedRole === role.key ? styles.roleCardActive : ''}`} key={role.key} onClick={() => setSelectedRole(role.key)} type="button">
      ...
      <article className={styles.card}>
        ...
        <div className={styles.matrixWrap}>
          <table className={styles.matrix}>
            ...
            <tbody>
              {PERMISSIONS.map((perm) => (<tr key={perm.resource}>
                  <td>
                    {perm.resource}
                    {perm.highRisk ? (<span className={styles.highRisk}>HIGH</span>) : null}
                  </td>
                  <td>
      ...
      <article className={styles.card}>
        ...
        <ul className={styles.activityList}>
          {RECENT_ACTIVITY.map((item) => {
            ...
            return (<li className={styles.activityItem} key={`${item.time}-${item.title}`}>
                <span aria-hidden="true" className={`${styles.activityIcon} ${styles[`activityIcon_${item.tone}`]}`}>
                  <Icon size={14}/>
                </span>
...
function AccessChip({ value }) {
    return (<span className={`${styles.accessChip} ${styles[`access_${value}`]}`}>
      {value === 'allow' ? (<Check aria-hidden="true" size={11}/>) : value === 'deny' ? (<X aria-hidden="true" size={11}/>) : (<CircleDot aria-hidden="true" size={11}/>)}
