...
export function NoticesPage() {
    ...
    const filteredNotices = useMemo(() => {
        ...
        return searchableNotices.filter((notice) => notice.searchableText.includes(normalizedSearchKeyword));
    ...
    return (<>
      ...
      <section className={styles.listSection}>
        <Container>
          <div className={styles.listHeader}>
            ...
            : `총 ${notices.length}건`}
              ...
              <div className={styles.searchBox}>
                ...
                <input aria-label="공지 검색" onChange={(event) => setSearchKeyword(event.target.value)} placeholder="공지 검색" type="search" value={searchKeyword}/>
                {hasSearchKeyword ? (<button aria-label="검색어 지우기" className={styles.clearSearch} onClick={() => setSearchKeyword('')} type="button">
          ...
          <NoticeList getCategoryLabel={getCategoryLabel} isLoading={isLoading} notices={visibleNotices}/>

          {!isLoading && hasMoreNotices ? (<div className={styles.loadMoreWrap}>
              <button className={styles.loadMore} onClick={() => setVisibleCount((count) => count + NOTICES_PER_PAGE)} type="button">
