        ...
        <Container>
          <div className={styles.heroOverlay}>
            <div className={styles.heroChips}>
              {heroChips.map((c) => (<span className={`${styles.chip} ${c.primary ? styles.chipPrimary : ''}`} key={c.label}>
                  {c.label}
                </span>))}
            </div>
      ...
      <section className={styles.titleBlock}>
        <Container>
          <div className={styles.titleChips}>
            {titleChips.map((c) => (<span className={`${styles.chip} ${c.primary ? styles.chipPrimaryStrong : styles.chipMuted}`} key={c.label}>
                {c.label}
              </span>))}
          </div>
      ...
      <section className={styles.glance}>
        <Container>
          <ul className={styles.glanceGrid}>
            {glance.map((g) => (<li className={`${styles.glanceCard} ${g.highlight ? styles.glanceCardHi : ''}`} key={g.label}>
      ...
      <section className={styles.approach}>
        <Container>
          ...
          <ol className={styles.approachList}>
            {approachSteps.map((s, i) => (<li className={styles.approachItem} key={s.no}>
                <div className={styles.approachMarker}>
                  <span className={styles.approachMarkerNo}>{s.no}</span>
                  {i < approachSteps.length - 1 ? (<span aria-hidden="true" className={styles.approachLine}/>) : null}
                </div>
        ...
        <Container>
          ...
          <div className={styles.gallery}>
            <article className={styles.galleryCard}>
              <div className={styles.galleryImage} style={{
            backgroundImage: 'linear-gradient(146deg, rgb(242, 237, 224) 0%, rgb(217, 199, 173) 71%)',
        }}>
            ...
            <article className={styles.galleryCard}>
              <div className={styles.galleryImage} style={{
            backgroundImage: 'linear-gradient(146deg, rgb(235, 240, 247) 0%, rgb(199, 212, 232) 71%)',
        }}>
      ...
      <section className={styles.results}>
        <Container>
          ...
          <ul className={styles.resultsGrid}>
            {results.map((r) => (<li className={styles.resultCard} key={r.label}>
                <div className={styles.resultTopRow}>
                  <span className={styles.resultValue}>{r.value}</span>
                  <span className={`${styles.resultDelta} ${r.deltaTone === 'success' ? styles.resultDeltaSuccess : styles.resultDeltaPrimary}`}>
                    {r.delta}
                  </span>
                </div>
          ...
          <div className={styles.chartCard}>
            ...
            <div className={styles.chart}>
              ...
              <div className={styles.chartBars}>
                {chartBars.map((b, i) => (<div className={styles.chartCol} key={b.week}>
                    <span className={styles.chartVal}>{b.value}%</span>
                    <span className={`${styles.chartBar} ${b.after ? styles.chartBarAfter : styles.chartBarBefore}`} style={{ height: `${b.height}%` }}/>
                    <span className={styles.chartWeek}>{b.week}</span>
                    {i === 3 ? (<span aria-hidden="true" className={styles.chartLaunch}>
      ...
      <section className={styles.tech}>
        <Container>
          ...
          <ul className={styles.techGrid}>
            {techStack.map((g) => (<li className={styles.techGroup} key={g.label}>
                <span className={styles.techLabel}>{g.label}</span>
                <div className={styles.techPills}>
                  {g.items.map((it) => (<span className={styles.techPill} key={it}>
                      {it}
                    </span>))}
                </div>
              </li>))}
          ...
          <div className={styles.teamGrid}>
            <div className={styles.teamCard}>
              ...
              <ul className={styles.teamRoles}>
                {teamRoles.map((r, i) => (<li className={`${styles.teamRole} ${i < teamRoles.length - 1 ? styles.teamRoleBorder : ''}`} key={r.role}>
            ...
            <div className={styles.teamCard}>
              ...
              <div className={styles.timelineBar}>
                {timelinePhases.map((p) => (<span className={`${styles.timelineSeg} ${styles[`timelineSeg_${p.color}`] ?? ''}`} key={p.label} style={{ width: `${p.width}%` }}/>))}
              </div>
              ...
              <ul className={styles.timelineLegend}>
                {timelinePhases.map((p) => (<li className={styles.timelineLegendItem} key={p.label}>
                    <span className={`${styles.timelineLegendDot} ${styles[`timelineSeg_${p.color}`] ?? ''}`}/>
                    {p.label}
                  </li>))}
              </ul>
            </div>
      ...
      <section className={styles.related}>
        <Container>
          ...
          <ul className={styles.relatedGrid}>
            {relatedCases.map((c) => (<li className={styles.relatedItem} key={c.title}>
