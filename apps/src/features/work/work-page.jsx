...
function FeaturedCard({ data, size }) {
    const isLarge = size === 'large';
    return (<article className={`${styles.featuredCard} ${isLarge ? styles.featuredCardLarge : ''}`}>
      ...
      <div className={styles.featuredBody}>
        ...
        <h3 className={`${styles.featuredTitle} ${isLarge ? styles.featuredTitleLarge : ''}`}>
          {isLarge
            ? data.title.split('\n').map((line, i) => (<span className={styles.featuredTitleLine} key={i}>
                  {line}
                </span>))
            : data.title}
        </h3>
...
function CaseCard({ data }) {
    return (<article className={`${styles.caseCard} ${data.size === 'tall' ? styles.caseCardTall : ''}`}>
        ...
        <Container>
          <div className={styles.heroHead}>
            ...
            <ul className={styles.heroStats}>
              {heroStats.map((s) => (<li className={styles.heroStatChip} key={s.label}>
                  {s.label}
                </li>))}
            </ul>
          </div>
        ...
        <Container>
          ...
          <div className={styles.featuredGrid}>
            ...
            <div className={styles.featuredColSmall}>
              {featuredSmall.map((c) => (<FeaturedCard data={c} key={c.title} size="small"/>))}
            </div>
          </div>
        </Container>
      </section>
      ...
      <section className={styles.filter}>
        <Container>
          <div className={styles.filterRows}>
            {filterGroups.map((g) => (<div className={styles.filterRow} key={g.label}>
                <span className={styles.filterLabel}>{g.label}</span>
                <div className={styles.filterChips}>
                  {g.options.map((opt, i) => (<button className={`${styles.filterChip} ${i === 0 ? styles.filterChipActive : ''}`} key={opt} type="button">
                      {opt}
                    </button>))}
                </div>
                {g.label === '연도' ? (<span className={styles.filterTotal}>
      ...
      <section className={styles.allCases}>
        <Container>
          <div className={styles.allGrid}>
            {allCases.map((c, i) => (<CaseCard data={c} key={`${c.title}-${i}`}/>))}
          </div>
      ...
      <section className={styles.statsStrip}>
        <Container>
          <ul className={styles.statsList}>
            {stats.map((s) => (<li className={styles.statItem} key={s.label}>
