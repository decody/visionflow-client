...
import { BarChart3, Briefcase, FileQuestionMarkIcon, FileText, Handshake, LayoutDashboard, MessageCircleQuestionMarkIcon, MessageSquareWarningIcon, MessagesSquare, Settings, Users, } from 'lucide-react';
    ...
    return (<header className={styles.header}>
      <aside className={styles.sidebar}>
        ...
        <nav aria-label="관리자 메뉴" className={styles.nav}>
          {NAV_SECTIONS.map((section) => (<div className={styles.navSection} key={section.title}>
              <p className={styles.sectionLabel}>{section.title}</p>
              <ul className={styles.navList}>
                {section.items.map((item) => {
                const Icon = item.icon;
                const isActive = item.href === ROUTES.ADMIN.HOME
                    ? pathname === ROUTES.ADMIN.HOME
                    : typeof pathname === 'string' &&
                        pathname.startsWith(item.href);
                return (<li key={item.href}>
                      <Link aria-current={isActive ? 'page' : undefined} className={styles.navLink} href={item.href}>
