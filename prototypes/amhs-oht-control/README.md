...
델타 대역폭은 **좌표 양자화**로 줄였다. 저장 물리 상태는 full precision을 유지하고 직렬화되는 델타·스냅샷
값만 x/y 1cm(소수 2자리)·heading 0.1도·speed 0.01m/s로 반올림한다(`quantizeWireFields`, engine.ts).
5,000대 델타가 214KB→155KB(−28%)로 줄었다. 스냅샷·델타가 동일 함수로 양자화하므로 재현 불변식은 유지된다.
