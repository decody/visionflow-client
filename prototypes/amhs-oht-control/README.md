...
- **가·감속·정지거리**: 순간 속도 변경 대신 물리적 가속도 프로파일(가속 2.0 m/s²·감속 1.6 m/s²)로 주행한다. 출발 시 0에서 가속하고, 목적지 접근 시 남은 주행거리의 정지거리(√(2·감속·거리))로 목표속도를 제한해 정차 지점 앞에서 감속한다. 속도는 항상 cruise 이하라 틱당 이동 상한(teleport 방지)이 유지된다.
